...
### MCP Local Bridge
ACK can expose its bookmark-graph tools to external MCP clients via a local **Native Messaging + MCP server**.
See `native/README.md` for setup and the MCP endpoint.
...
### MCP 本地桥接
ACK 也可以通过本地 **Native Messaging + MCP 服务端**把书签图谱能力暴露给外部 MCP 客户端。
安装与 MCP 地址请参考 `native/README.md`。